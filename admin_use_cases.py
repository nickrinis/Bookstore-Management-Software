    ...
    for ID, bookstores in bdata.itertuples(index=False):
        if delete_id == ID:
            ...
            if admin_bookstores == book_bookstores:
                ...
                index = i[0]
                # dropping the row at the index of the book
...
def generateGraph(user_df, books_df):
    ...
    while not menu == 0:
        ...
        if menu == 1:
            ...
            # getting bookstore index from the list for each bookstore and adding the copies (values) to the same index for the book list
            for ID, bookstores in bdata.itertuples(index=False):
                bookstores = ast.literal_eval(str(bookstores))
