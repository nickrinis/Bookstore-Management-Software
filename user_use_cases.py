...
def csvEntry(user_df, books_df, logged_in):
    ...
    for ID, favorites in udata.itertuples(index=False):
        if ID == logged_in:
            ...
            loops = int(input("Please enter the amount of entries you would like to add to your favorites: "))
    ...
    for ID, balance in udata.itertuples(index=False):
        if ID == logged_in:
            while correct_book == 0:
                ...
                book_title = input("-Enter the title of the book you wish to check the amount of copies you can purchase: ")
                for ID, title, cost, shipping_cost, availability, copies in bdata.itertuples(index=False):
                    ...
                    if book_title == title:
                        if availability:
                            ...
                            basket = 0
                            return_copies = 0
                            # while our basket with the would be added copy is less or equal to the balance and the copy number does not exceed the book copies
...
def getSuggestions(user_df, books_df, logged_in):
    ...
    for ID, orders, favorites in udata.itertuples(index=False):
        if ID == logged_in:
            ...
            # checking which category has the max number and assigning the chosen category to a variable
